...
| P6 | 進行中 | Gitなし対応と堅牢化 | T601〜T608 | P1〜P5 | T601〜T606はmainへ統合済み。T606はPR #77のsquash merge `2afa1b6a8299b2d25a1ef2c7186508028bbd5fb6`、exact-head CI `32432473407` Green、all reviews closed。T607はattestation CIのproduction follow-upとnew attestation待ち、T608は未着手 |

## 現在位置

- T306 merge commit: `ec74b88c68df73acf84373eeaf2706fae2d1b6f0`
- 直近統合: T603 schema migration・破損隔離・回復（PR #53、merge commit `8cbdaa55`）
- 実装中タスク: T607 / Issue #79 / PR #80（`532620d` attestation CIがT504 focused immutable snapshot 2件で失敗し無効・再利用不可、R3 production delta Green）
- 次の工程: R3 production delta commit/push後の同一reviewerによるT607 finding-limited new attestation
- 後続候補: T608
...
- T607: attestation HEAD `532620dc5cc91b3117fcb993ead5fd68e4e38b01` のexact `pull_request` CI run `32445673275` / job `96664691395` はUnit、T405、T406、T304、T502、T503通過後、T504 focused immutable snapshot 2件で失敗し無効かつ再利用不可。R3 report `reports/issue-79-t607-ci-followup-r3-20260821131334.md` はoperation-start coherent snapshotのproduction delta、exact T504三ファイル15/15、T607 79/79、static gates passを記録。new attestation pending、exact-head `pull_request` CI held、Markdown wording unsupported。
- T608: 未着手
