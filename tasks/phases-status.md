...
- T610: R13/R14でstartup Global refreshをnon-blocking queueへ分離しCurrent Context startup後へ整列したが、Hostは最初のsuite marker前でtimeoutしたため反復実行を停止。性能workloadのCI除外policyを適用し、T506 restart fixtureをfile-open lifecycle drainへ同期した。
