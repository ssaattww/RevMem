...
- 次のタスク: T609 final unit base比較deltaを同一normal reviewerが閉じた後、same independent closure、exact-head CI、PR #82 squash mergeへ進む
...
| T609 | normal unit delta closure待ち | L | Git管理folderが開かれていればactive Git editorがなくても単一・multi-rootのrepositoryを決定できるようにし、VS Codeで選択されたfile encodingをGit revision mappingへ連動させ、Shift-JIS・UTF-8・UTF-8 BOM混在や未解決encodingをfile単位で隔離する | T202、T302、T402、T405、T605、T606 | focused 77/77、T609 Host全phase Green。current unit 23件中22件はbase exact-match、confirmed regression 0。current-onlyはWindows symlink fixture作成EPERM 1件。同一normal reviewerがこのdeltaを処分後、same independent closureへ進む |
