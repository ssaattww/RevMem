...
- 次のタスク: 通常review finding `T306-R1-P1` Highと`T306-R1-P2` Mediumを一括修正し、同じ通常reviewerのbounded closure verificationへ進む
- 実装状態: focused/CIはGreenだが、通常reviewで実UI/runtimeを迂回する自己完結Test APIと、hang時のbounded termination/cleanup不足を検出した。AC-14〜AC-17のclosureはfinding修正後に再判定する
...
- Pull Request: PR #45（draft、通常review指摘対応中、merge未実施）
...
| T306 | 進行中（通常review指摘対応中） | L | local base/headをPR相当として、diff両側操作から進捗UI更新までのExtension Host試験を追加する | T300〜T305 | AC-14〜AC-17を実UI/runtime操作で通す。focused sideに依存しないファイル全体確認・全解除、ユーザー除外の分母除外と別表示、rename-only、binaryを検証する。`T306-R1-P1` Highと`T306-R1-P2` Mediumの一括修正、closure verification、独立review、mergeを残す |
...
T305とT403はcurrent mainへ統合済み。T306の全範囲通常reviewは完了し、`T306-R1-P1` Highと`T306-R1-P2` Mediumを一括修正する。修正後は同じ通常reviewerのbounded closure verificationを行い、closure後に別reviewerの全範囲独立reviewを一度実施してmergeへ進む。T306完了まで新規product taskへ着手しない。T404とT602は依存解消済み、T505は未実装のため対象外とする。
