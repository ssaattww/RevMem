...
- 現在のタスク: Issue #92 / PR #94 `PR94-IFR-001` R2。command→別revision→exact returnのactual compositionを補完する
- 次のタスク: `PR94-IFR-003` R2でpresent snapshot evidence取得不能をfail closedにし、通常fix verificationへ戻る
...
| PR94-IFR-001 | R2実装中 / High | 0.5h以内 | PR command targetへauthoritative contentHashを保持し、Context/Global一致をfail closedで検証してsnapshotへwrite-throughする | PR94-CI-005 | production/hash/no-publish Complete。actual command→別revision→exact return compositionを補完する |
...
| PR94-IFR-003 | R2修正待ち / Medium | 0.5h以内 | invalid snapshotのauthoritative fail-closed契約を両design・production・actual local-Git fixtureで統一する | PR94-IFR-002 | content mismatchはComplete。present snapshot evidence取得不能のfallbackをfail closedへ修正する |
