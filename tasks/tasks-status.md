...
- 次のタスク: T306の通常reviewを全範囲で一度実施する。closure後に別reviewerの独立reviewを全範囲で一度実施し、mergeへ進む
...
| T306 | 進行中（通常review待ち） | L | local base/headをPR相当として、diff両側操作から進捗UI更新までのExtension Host試験を追加する | T300〜T305 | AC-14〜AC-17をUI操作で通す。focused sideに依存しないファイル全体確認・全解除、ユーザー除外の分母除外と別表示、rename-only、binaryを検証する。focused Extension Host試験はGreen、通常review・独立review・mergeを残す |
...
T305とT403はcurrent mainへ統合済み。T306のtest-first実装とfocused Extension Host Greenは完了し、次は全範囲の通常reviewを一度実施する。closure後に別reviewerの全範囲独立reviewを一度実施し、mergeへ進む。T306完了まで新規product taskへ着手しない。T404とT602は依存解消済み、T505は未実装のため対象外とする。
