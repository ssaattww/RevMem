...
- 次のタスク: T609 IFR001〜IFR006の同一normal reviewer fix verificationと同一independent reviewer finding限定closureを完了し、exact-head CI後にPR #82をsquash mergeする
...
- Pull Request: #82（draft。independent final review `reports/issue-81-t609-independent-final-review-20260822060225.md` のIFR001〜IFR006はtechnical head `5501cd7`でaddressed。tracking commit後にPR bodyを同headの事実へ外部同期し、同一reviewer closureへ進む）
- T609 reports: focused `npm run test:t609` 77/77、build/compile/lint/diffcheck Green。actual Extension Hostはsingle-root、multi-root prepare、restart-reopen、fixture cleanupの全phaseが成功。current handoffは `handoffs/issue-81-t609-independent-review-followup-20260822080000.yaml`、最新follow-up reportは `reports/issue-81-t609-independent-review-followup-r28-restart-mapping-20260822153933.md`。CIはmerge gateまで未待機。
...
| T609 | 独立finding closure待ち | L | Git管理folderが開かれていればactive Git editorがなくても単一・multi-rootのrepositoryを決定できるようにし、VS Codeで選択されたfile encodingをGit revision mappingへ連動させ、Shift-JIS・UTF-8・UTF-8 BOM混在や未解決encodingをfile単位で隔離する | T202、T302、T402、T405、T605、T606 | 通常review 7 findings closed。一度限りの独立review IFR001〜006をtechnical head `5501cd7`でaddressed。focused 77/77、Host single-root・prepare・restart-reopen・cleanupが全てGreen。同一normal reviewer verification、同一independent reviewer closure、exact-head CI後にsquash mergeする |
