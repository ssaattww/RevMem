...
- 次のタスク: 修正済みの通常review finding `T306-R1-P1` Highと`T306-R1-P2` Mediumを、同じ通常reviewerのbounded closure verificationで確認する
- 実装状態: 自己完結Test APIを廃止し、local base/head runtimeを実Tree View・virtual diff・既存command・dialog・永続state/historyへ接続した。Extension Host runnerにはlaunch単位の上限、owned process-tree停止、cleanup、redacted diagnosticを追加し、focused/default Extension Host試験はGreen。通常review finding closure待ち
...
T305とT403はcurrent mainへ統合済み。T306の全範囲通常reviewは完了し、`T306-R1-P1` Highと`T306-R1-P2` Mediumは実装側で修正済み。同じ通常reviewerのbounded closure verificationを行い、closure後に別reviewerの全範囲独立reviewを一度実施してmergeへ進む。T306完了まで新規product taskへ着手しない。T404とT602は依存解消済み、T505は未実装のため対象外とする。
