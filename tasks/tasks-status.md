...
- 次のタスク: T609の設計契約を更新し、アクティブGit editorがない場合のrepository選択とShift-JIS・UTF-8・UTF-8 BOM混在時のfile単位fail-closedをTDDで実装する
...
| T609 | 設計更新中 | L | Git管理folderが開かれていればactive Git editorがなくても単一・multi-rootのrepositoryを決定できるようにし、VS Codeで選択されたfile encodingをGit revision mappingへ連動させ、Shift-JIS・UTF-8・UTF-8 BOM混在や未解決encodingをfile単位で隔離する | T202、T302、T402、T405、T605、T606 | active editorなし・非Git editor・単一root・multi-root選択が動き、opened Shift-JIS fileとGit revision blobを同一encodingでmappingできる。未知または不正encodingは該当fileだけをline-review対象外にしてReview Contexts全体を停止せず、rename・設定変更・restart・privacy-safe診断を含むfocused、統合、Extension Host testと通常・独立reviewを通過する |
