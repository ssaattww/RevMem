...
- 次のタスク: T609のreview-target commitを作成し、sol highによる一度の全範囲通常reviewへ進む
...
- Pull Request: #82（draft。設計commit `68951cd`を公開済み。production実装とlocal TDDは完了、通常review・独立review・final exact-head CIは未完了）
- T609 reports: `reports/issue-81-t609-design-20260822003958.md`、`reports/issue-81-t609-implementation-20260822004548.md`、`reports/issue-81-t609-implementation-r2-20260822011012.md`。focused 28件、actual composition 1件、build、compile:test、contracts、architecture正負、lint、diff-check成功
...
| T609 | 実装Green・通常review待ち | L | Git管理folderが開かれていればactive Git editorがなくても単一・multi-rootのrepositoryを決定できるようにし、VS Codeで選択されたfile encodingをGit revision mappingへ連動させ、Shift-JIS・UTF-8・UTF-8 BOM混在や未解決encodingをfile単位で隔離する | T202、T302、T402、T405、T605、T606 | active editorなし・非Git editor・単一root・multi-root選択が動き、opened Shift-JIS fileとGit revision blobを同一encodingでmappingできる。未知または不正encodingは該当fileだけをline-review対象外にしてReview Contexts全体を停止せず、rename・設定変更・restart・privacy-safe診断を含むfocused、統合、Extension Host testと通常・独立reviewを通過する。local evidenceはfocused 28件、actual composition 1件、build・compile:test・contracts・architecture正負・lint・diff-check Green。通常review以降はpending |
