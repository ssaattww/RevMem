...
- open finding: なし。
- closed finding（初回identity / severityと発見時履歴を維持）:

  1. `USR90-002-R2-IFR-001` — **Low / tracking and report accuracy / closed**
     ...
     - evidence / disposition: 発見時、`gh pr view 91`はhead `8cadc8431a59358a88902f87d582b373a5b547f6`を返し、exact `e996337...`はremote commit未存在 / matching CI absentだったため、required report / tracking accuracy cellをIncompleteとしてopenした。`40d753d...`の4-file fixはremote / CI / artifact identity、R2A/B normal closure、held scopeを同期し、同じnormal reviewerが限定verificationでrequired action / focused evidenceをComplete、新規findingなし、verdict=`pass_with_held`と確認した。frozen `200f16e...`もremote未存在でexact-head CI / artifactなし、prior artifact非転用を維持する。**closed**。

  2. `PR91-IFR-001` — **High / required / closed**
  ...
  3. `PR91-IFR-002` — **Medium / required / closed**
  ...
  4. `CI90-001` — **CI delta / required / closed**
  ...
  5. `CI90-002` — **CI delta / required / closed**
  ...
  6. `USR90-002-NR-001` — **Low / evidence/tracking accuracy / closed**
  ...
  7. `USR90-002-R2-NR-001` — **High / blocking normal-path / closed**
  ...
  8. `USR90-002-R2-NR-002` — **Medium / blocking review-evidence gap / closed**
...
- required coverage dispositions:
  ...
  - USR90-002-R2 report / tracking accuracy=`checked_no_finding`（`USR90-002-R2-IFR-001` Low closed。remote / CI / artifact identity、R2A/B review state、held scopeを4-file fixと同じnormal reviewer reportで同期）
