...
- 実装検証commit: `03c059a459d5b1996008821ab5e57c044b940042`
...
- 原因調査に必要な情報が既に含まれていたためworkflow変更は行わなかった
...
- URI形式: `review-range-diff://document/v1/{context}/{side}/{revision}/{file}`
- 可変fieldはcanonical base64urlで保持し、Unicode、空白、path separatorを可逆化した
- scheme、authority、version、segment数、side、query、fragment、userinfo、portを厳密検証した
- control character、非canonical base64url、不正UTF-8、unpaired UTF-16 surrogate、過大入力を決定的に拒否した
...
- unsafe revision optionとroot escape・absolute path・backslash pathを拒否した
...
- GitHub APIとsnapshotによるcontent fallbackはT401以降で実装する
