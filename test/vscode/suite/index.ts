...
const expectedThemeColors = new Map([
  ["reviewRange.reviewedBackground", { dark: "#a0a0a01f", light: "#6060601f", highContrast: "#a0a0a033", highContrastLight: "#60606033" }],
  ["reviewRange.reviewedOverviewRuler", { dark: "#a0a0a08c", light: "#6060608c", highContrast: "#a0a0a0cc", highContrastLight: "#606060cc" }]
]);

const TEST_PHASE_ENVIRONMENT_VARIABLE = "REVIEW_RANGE_TEST_PHASE";
type TestPhase = "confirm" | "restore-confirmed-and-unmark" | "restore-unmarked";
...
const readTestPhase = (): TestPhase => {
  ...
  assert.ok(
    phase === "confirm" || phase === "restore-confirmed-and-unmark" || phase === "restore-unmarked",
...
const assertManifestAndConfiguration = async (
  ...
  for (const [configurationKey, expectedDefault] of Object.entries(expectedDecorationDefaults)) {
    assert.equal(configurationProperties[configurationKey].default, expectedDefault);
  }
  assert.deepEqual(configurationProperties["reviewRange.exclude"].default, expectedExcludeGlobs);

  const contributedColors = new Map(
    extension.packageJSON.contributes.colors.map(
      (color: { readonly id: string; readonly defaults: unknown }) => [color.id, color.defaults]
    )
  );
  for (const [colorId, expectedDefaults] of expectedThemeColors) {
    assert.deepEqual(contributedColors.get(colorId), expectedDefaults);
  }
  ...
  for (const commandId of expectedCommandIds) assert.equal(registeredCommands.has(commandId), true);

  const configuration = vscode.workspace.getConfiguration("reviewRange");
  for (const [configurationKey, expectedDefault] of Object.entries(expectedDecorationDefaults)) {
    assert.equal(configuration.get(configurationKey.replace("reviewRange.", "")), expectedDefault);
  }
  assert.deepEqual(configuration.get("exclude"), expectedExcludeGlobs);
};
...
const waitForRevision = async (
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (predicate(revision)) return revision;
...
const assertExclusionConfigurationLifecycle = async (
  ...
  await configuration.update("exclude", configuredGlobs, vscode.ConfigurationTarget.Workspace);
  const configuredRevision = await waitForRevision(extensionApi, (revision) => revision > initial.revision);
  assert.equal(extensionApi.evaluateFileExclusion("src/model.generated.ts").excluded, true);

  await configuration.update("showOverviewRuler", true, vscode.ConfigurationTarget.Workspace);
  await new Promise((resolve) => setTimeout(resolve, 30));
  assert.equal(extensionApi.getFileExclusionPolicySnapshot().revision, configuredRevision);
  await configuration.update("showOverviewRuler", undefined, vscode.ConfigurationTarget.Workspace);
  ...
  await waitForRevision(extensionApi, (revision) => revision > configuredRevision);
  assert.deepEqual(extensionApi.getFileExclusionPolicySnapshot().userGlobs, expectedExcludeGlobs);
};

const openLifecycleFixture = async (
  phase: TestPhase,
  workspaceFolder: vscode.WorkspaceFolder
): Promise<{ readonly documentUri: vscode.Uri; readonly editor: vscode.TextEditor; readonly splitEditor: vscode.TextEditor }> => {
  const documentUri = vscode.Uri.joinPath(workspaceFolder.uri, "lifecycle-restart.ts");
  if (phase === "confirm") {
    await vscode.workspace.fs.writeFile(documentUri, Buffer.from("const first = 1;\nconst second = 2;\n"));
  } else {
  ...
  const splitEditor = await vscode.window.showTextDocument(document, { viewColumn: vscode.ViewColumn.Beside, preview: false });
  ...
  assert.ok(visibleCopies.length >= 2);
...
export async function run(): Promise<void> {
  ...
  const extensionApi = (await extension.activate()) as ReviewRangeExtensionTestApi | undefined;
  assert.equal(extension.isActive, true);
  assert.ok(extensionApi, "Test-mode activation should expose runtime observation hooks.");
  await assertManifestAndConfiguration(extension);
  if (phase === "confirm") await assertExclusionConfigurationLifecycle(extensionApi);

  const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
  assert.ok(workspaceFolder, "The Extension Host fixture should open a workspace folder.");
  const { documentUri, editor, splitEditor } = await openLifecycleFixture(phase, workspaceFolder);
  ...
  if (phase === "restore-confirmed-and-unmark") {
    ...
    await extensionApi.refreshVisibleEditorDecorations();
    assert.deepEqual(extensionApi.getVisibleReviewedIntervals(documentUri.toString()), []);
    return;
  }
