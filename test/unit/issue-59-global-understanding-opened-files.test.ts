...
test("Issue #59 Global denominator keeps opened-file evidence and does not scan unopened file contents", async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), "review-range-issue-59-"));
  ...
  assert.equal(whileOpen?.progress.files.length, 1);
  assert.equal(whileOpen?.progress.files[0]?.path, "opened.ts");
  ...
  assert.equal(afterClose?.progress.files.length, 1);
  assert.equal(afterClose?.progress.files[0]?.path, "opened.ts");
