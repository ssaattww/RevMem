...
import {
  ...
  NodeGitHubPullRequestContextLayerStore,
...
function layer(overrides: Partial<GitHubPullRequestContextLayer> = {}): GitHubPullRequestContextLayer {
  return {
    contextId: createGitHubPullRequestContextId({
      host: "github.com",
      owner: "ssaattww",
      repository: "RevMem",
    ...
    files: {
      "src/example.ts": [{ startLine: 1, endLineExclusive: 3 }],
    },
    ...overrides,
  };
}
...
test("commit追加でも同じPR context IDを継続してrevisionだけ更新する", async () => {
  ...
  const store = new NodeGitHubPullRequestContextLayerStore(root);
  const initial = layer();
  await store.upsert(initial);

  const updated = await store.upsert(layer({ headRevision: SHA_C, updatedAt: "2026-08-06T09:50:00.000Z" }));

  assert.equal(updated.contextId, initial.contextId);
  assert.equal(updated.headRevision, SHA_C);
  assert.deepEqual(updated.files, initial.files);
});

test("別PRは別layerとして保存し再起動後も復元する", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "revmem-t404-"));
  const store = new NodeGitHubPullRequestContextLayerStore(root);
  await store.upsert(layer());
  await store.upsert(layer({
    contextId: createGitHubPullRequestContextId({ host: "github.com", owner: "ssaattww", repository: "RevMem", pullRequestNumber: 49 }),
    pullRequestNumber: 49,
  }));

  const restarted = new NodeGitHubPullRequestContextLayerStore(root);
  const restored = await restarted.list();

  assert.equal(restored.length, 2);
  assert.notEqual(restored[0]?.contextId, restored[1]?.contextId);
});
...
test("closedまたはmerged PRは既定で装飾を無効化する", async () => {
  ...
  const closed = await store.upsert(layer({ state: "closed", decorationEnabled: true }));
  const merged = await store.upsert(layer({ state: "merged", decorationEnabled: true }));

  assert.equal(closed.decorationEnabled, false);
  assert.equal(merged.decorationEnabled, false);
});
...
test("globalStorage配下にはsource本文やtokenを含まないversioned JSONだけを保存する", async () => {
  ...
  const persisted = await readFile(path.join(root, "github-pr-context-layers.v1.json"), "utf8");
  assert.match(persisted, /\"version\":1/);
  assert.doesNotMatch(persisted, /token|sourceText|authorization/i);
});
