...
} from "../../src/application/file-exclusion/index";

const candidate = (
  path: string,
  isBinary = false
): ReviewFileExclusionCandidate => ({ path, isBinary });

const expectedDefaultGlobs = [
  "**/.git/**",
  "**/node_modules/**",
  "**/bin/**",
  "**/obj/**",
...
test("every default glob excludes root and nested generated directories with a reason", () => {
  ...
  const cases = [
    [".git/config", "**/.git/**"],
    ["packages/app/.git/config", "**/.git/**"],
    ["node_modules/pkg/index.js", "**/node_modules/**"],
    ["packages/app/node_modules/pkg/index.js", "**/node_modules/**"],
    ["bin/app.dll", "**/bin/**"],
    ["src/bin/app.dll", "**/bin/**"],
    ["obj/cache.json", "**/obj/**"],
    ["src/obj/cache.json", "**/obj/**"],
    ["dist/index.js", "**/dist/**"],
    ["packages/app/dist/index.js", "**/dist/**"],
    ["build/output.txt", "**/build/**"],
    ["packages/app/build/output.txt", "**/build/**"]
  ] as const;

  for (const [path, pattern] of cases) {
    assert.deepEqual(policy.evaluate(candidate(path)), {
      excluded: true,
      normalizedPath: path,
      reason: { kind: "default-glob", pattern }
    });
  }

  assert.deepEqual(policy.evaluate(candidate("src/binocular/index.ts")), {
    excluded: false,
    normalizedPath: "src/binocular/index.ts"
  });
});

test("repository-relative paths are normalized without changing Git case semantics", () => {
  const policy = new ReviewFileExclusionPolicy({
    userGlobs: ["src/**/*.generated.ts"]
  });

  assert.deepEqual(policy.evaluate(candidate(".\\src\\models\\item.generated.ts")), {
    excluded: true,
    normalizedPath: "src/models/item.generated.ts",
    reason: { kind: "user-glob", pattern: "src/**/*.generated.ts" }
  });
  assert.deepEqual(policy.evaluate(candidate("Src/models/item.generated.ts")), {
    excluded: false,
    normalizedPath: "Src/models/item.generated.ts"
...
test("user globs support basename, recursive, question, class, and brace patterns", () => {
  ...
  for (const path of [
    ...
    "src/deep/model.generated.tsx",
    "src/fixture1.js",
    "src/deep/fixtureA.ts"
  ]) {
    assert.equal(policy.evaluate(candidate(path)).excluded, true, path);
  }
  ...
  for (const path of [
    ...
    assert.equal(policy.evaluate(candidate(path)).excluded, false, path);
  }
});
...
test("user glob normalization removes blanks and semantic duplicates", () => {
  ...
  assert.deepEqual(policy.evaluate(candidate("src/generated/file.ts")), {
    excluded: true,
    normalizedPath: "src/generated/file.ts",
    reason: { kind: "user-glob", pattern: "**/generated/**" }
  });
...
test("policy rejects non-repository paths and unsupported negated globs", () => {
  assert.throws(
    () => new ReviewFileExclusionPolicy({ userGlobs: ["!**/*.ts"] }),
    /negated glob/i
  );

  const policy = new ReviewFileExclusionPolicy();
  for (const path of ["../outside.ts", "/absolute.ts", "C:/absolute.ts", "src/\u0000bad.ts"]) {
    assert.throws(() => policy.evaluate(candidate(path)), /repository-relative path/i);
  }
});

test("the first matching glob provides a deterministic exclusion reason", () => {
  const policy = new ReviewFileExclusionPolicy({
    userGlobs: ["**/*.generated.ts", "src/**"]
  });

  assert.deepEqual(policy.evaluate(candidate("src/model.generated.ts")), {
    excluded: true,
    normalizedPath: "src/model.generated.ts",
    reason: { kind: "user-glob", pattern: "**/*.generated.ts" }
  });
});
...
test("settings updates replace the shared policy and notify only on semantic changes", () => {
  ...
  const disposable = service.onDidChange((event) => {
    events.push(event);
  });
  ...
  assert.deepEqual(events, [
    { revision: 1, userGlobs: ["**/*.generated.ts"] }
  ]);
...
test("PR and Global consumers can reuse one service and receive identical decisions", () => {
  ...
  assert.deepEqual(prProgressDecision, {
    excluded: true,
    normalizedPath: "src/file.generated.ts",
    reason: { kind: "user-glob", pattern: "**/*.generated.ts" }
  });
});
...
test("package manifest exposes the designed reviewRange.exclude default", () => {
  const manifest = JSON.parse(
    ...
    contributes: {
      configuration: {
        properties: Record<string, { readonly type: string; readonly default: unknown }>;
      };
    };
  };

  assert.deepEqual(
    manifest.contributes.configuration.properties["reviewRange.exclude"],
    {
      type: "array",
      items: { type: "string" },
      default: expectedDefaultGlobs,
      description: "PR進捗とGlobal理解率の集計対象から除外するファイルglobです。"
    }
  );
});
