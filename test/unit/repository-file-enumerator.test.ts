...
test("enumerates deterministic included files and preserves exclusion reasons", async () => {
  ...
  assert.deepEqual(result.included.map((file) => file.nonEmptyLineCount), [2, 2, 2]);
  assert.equal(result.excluded.find((file) => file.path === "dist/bundle.js")?.reason.kind, "default-glob");
  assert.equal(result.excluded.find((file) => file.path === "ignored/secret.ts")?.reason.kind, "gitignore");
