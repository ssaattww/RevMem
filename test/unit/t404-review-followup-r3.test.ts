...
class MemoryRepository implements GitHubPullRequestContextRepositoryPort { current?: PullRequestReviewStateCommit; commits = 0; async load(): Promise<PullRequestReviewStateCommit | undefined> { return this.current === undefined ? undefined : structuredClone(this.current); } async create(input: Parameters<GitHubPullRequestContextRepositoryPort["create"]>[0]): Promise<void> { this.current = structuredClone(input.next); } async commit(input: Parameters<GitHubPullRequestContextRepositoryPort["commit"]>[0]): Promise<void> { this.commits += 1; this.current = structuredClone(input.next); } }
